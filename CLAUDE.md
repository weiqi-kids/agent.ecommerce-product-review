...
| **Reviewer** | 獨立審核每個 Step 完成度 | `core/Architect/reviewer/CLAUDE.md` |
...
| 平台 | 狀態 | 停用日期 | 原因 |
|------|------|---------|------|
| **walmart_us** | ❌ 永久停用 | 2026-03-03 | Selector 連續 4+ 次全面失效，使用者決定不再使用 |

> ⚠️ **Walmart 已永久停用**：本文件中仍有 Walmart 相關描述（歷史記錄），但執行時**一律跳過** walmart_us。不需要詢問使用者是否啟用 Walmart，也不需要嘗試修復 Walmart selector。
...
> ⚠️ **禁止中途確認**：執行完整流程時，**不可**停下來詢問使用者「是否繼續」「要執行哪些類別」等問題。Step 1-8 + 執行結束必做事項必須一口氣完成，最後才用品質關卡檢查。
...
Step 1: 監控清單追蹤（平行執行）
        ...
        ↓
    ▶ Reviewer 審核 Step 1 → 通過才進 Step 2
        ↓
Step 2: 研究缺口補齊（平行執行）
        ...
        ├── 補齊後重新執行 Step 7-8
        └── 產出：docs/Extractor/gap_analysis_{date}.json
        ↓
    ▶ Reviewer 審核 Step 2 → 通過才進 Step 3
        ↓
Step 3: 抓取排行榜（多平台平行，增量模式）
        ...
        ├── 跨平台 UPC 去重
        ├── 合併為熱門產品清單（含來源標註）
        └── 產出：docs/Extractor/discovery_cache/{date}.json
        ↓
    ▶ Reviewer 審核 Step 3 → 通過才進 Step 4
        ↓
Step 4: 產品分組（按具體問題）
        ...
        └── 產出：docs/Extractor/grouping_result_{date}.json
        ↓
    ▶ Reviewer 審核 Step 4 → 通過才進 Step 5
        ↓
Step 5: 問題研究 + 競品發現
        ├── 對每個問題類別執行深度研究
...
Step 6: 抓取評論 + 萃取（多平台 + 社群來源）
        ...
        └── 產出：docs/Extractor/fetch_result_{date}.json
        ↓
    ▶ Reviewer 審核 Step 6 → 通過才進 Step 7
        ↓
Step 7: 比較分析（多平台 + 多來源）
        ├── 輸入：
        ...
        └── 產出：docs/Extractor/comparison_analysis_{類別}_{date}.json
        ↓
    ▶ Reviewer 審核 Step 7 → 通過才進 Step 8
        ↓
Step 8: 產出報告（類型依分析結果，不可跳過）
        ...
        ↓
    ▶ Reviewer 審核 Step 8 → 通過才進 Step 9
        ↓
...
每個 Step 完成後，**必須**由獨立的 Reviewer 子代理審核，通過才能進入下一步。
...
啟動 Reviewer 子代理
        ↓
Reviewer 檢查產出物是否存在
Reviewer 驗證產出物格式是否正確
Reviewer 比對數量是否達標
        ↓
    ├── 全部通過 → 進入 Step N+1
    ├── 部分未通過（可補救）→ 執行補救動作 → 重新審核
    └── 嚴重未通過（需重做）→ 重做 Step N
```
...
Task(
  ...
  prompt: "依據 core/Architect/reviewer/CLAUDE.md 審核 Step {N} 是否完成。
          只輸出審核報告，不執行任何修改。
          審核範圍：檢查項目 {N}.1 - {N}.X",
...
| Step | 必要產出物 |
|------|-----------|
| 1 | watchlist.json 更新（或無到期項目記錄） |
| 2 | gap_analysis_{date}.json |
| 3 | discovery_cache/{date}.json |
| 4 | problem_groups.md 更新 + grouping_result_{date}.json |
...
> 詳細審核項目見 `core/Architect/reviewer/CLAUDE.md`。
...
| **R** | **Reviewer 審核** | `haiku` | `general-purpose` | 輕量驗證任務 |

**強制規則**：
- **只有 Step 8**（報告產出）使用 `opus`，其餘一律 `sonnet`
- **Reviewer 審核**使用 `haiku`（輕量、低成本）
...
執行完整流程時，必須控制 Token 使用量以避免 context overflow。

#### Task 子代理輸出限制

| Step | 任務類型 | 最大輸出行數 | 說明 |
|------|---------|-------------|------|
| 1 | 監控追蹤 | 30 行 | 只回報狀態變更，無變化只說「無變化」 |
| 3 | Discovery | 30 行 | 只回報數量統計，不列出所有產品 |
| 4 | 產品分組 | 50 行 | 只回報分組統計，不列出所有產品 |
| 6 | 萃取結果 | 20 行 | 只回報成功/失敗統計 |
| 7 | 比較分析 | 30 行 | 只回報結論摘要 |
| 8 | 報告產出 | 20 行 | 只回報檔案路徑和報告類型 |
...
```bash
# 限制 Discovery 輸出為前 30 筆
head -30 docs/Extractor/discovery_cache/{date}.json
```

**禁止行為**：
- ❌ 在對話中列出所有發現的產品
- ❌ 在 Task 結果中包含完整 JSON 陣列

**正確做法**：
- ✅ 只回報數量統計：「發現 150 個產品（Amazon: 120, Walmart: 20, Best Buy: 10）」
- ✅ 詳細資料寫入檔案，不在對話中顯示
...
| 檔案類型 | 最大讀取行數 | 處理方式 |
|---------|-------------|---------|
| JSONL | 禁止直接讀取 | 使用 `sed -n '{N}p'` 逐行 |
| JSON（>200 行） | 100 行 | 使用 offset + limit |
| Markdown（>500 行） | 200 行 | 使用 offset + limit |
| discovery_cache | 30 行 | 只讀取統計區塊 |

#### 每日摘要精簡格式

每日摘要（`docs/daily_summary/{date}.md`）必須精簡：
- 總行數 ≤150 行
- 每個 Step 摘要 ≤10 行
- 只列出有變化的項目
