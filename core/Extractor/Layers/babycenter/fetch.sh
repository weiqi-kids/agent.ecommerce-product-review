  ...
  cat > "$output_file" << EOF
{
  ...
  "mode": "mcp_fetch",
  "forum_domain": "community.babycenter.com",
  "category": "baby",
  "search_queries": [
    "site:community.babycenter.com \"${query}\" review",
    "site:community.babycenter.com \"${query}\" impressions"
  ],
  "instructions": {
    "step1": "使用 WebSearch: site:community.babycenter.com \"${query}\"",
...
if [[ -n "$QUERY" ]]; then
  ...
  generate_fetch_plan "$QUERY" "$PLAN_DIR/babycenter-${SAFE_NAME}-$(date +%Y-%m-%d).json"
  exit 0
fi
...
if [[ -f "$QUERIES_FILE" ]]; then
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    generate_fetch_plan "$line" "$PLAN_DIR/babycenter-${SAFE_NAME}-$(date +%Y-%m-%d).json"
  done < "$QUERIES_FILE"
