  ...
  cat > "$output_file" << EOF
{
  ...
  "mode": "mcp_fetch",
  "forum_domain": "head-fi.org",
  "category": "electronics",
  "search_queries": [
    "site:head-fi.org \"${query}\" review",
    "site:head-fi.org \"${query}\" impressions"
  ],
  "instructions": {
    "step1": "使用 WebSearch: site:head-fi.org \"${query}\"",
...
if [[ -n "$QUERY" ]]; then
  ...
  generate_fetch_plan "$QUERY" "$PLAN_DIR/headfi-${SAFE_NAME}-$(date +%Y-%m-%d).json"
  exit 0
fi
...
if [[ -f "$QUERIES_FILE" ]]; then
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    generate_fetch_plan "$line" "$PLAN_DIR/headfi-${SAFE_NAME}-$(date +%Y-%m-%d).json"
  done < "$QUERIES_FILE"
