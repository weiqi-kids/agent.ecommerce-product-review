...
### 執行結束後未主動完成收尾工作（2026-02-21）

**錯誤**：執行完整流程後，沒有主動記錄錯誤經驗，等使用者提醒才補做。

**正確做法**：

1. **執行結束必做事項應完全自主完成**，包括：
   - 更新 watchlist.json
...
2. **不需要使用者授權或提醒的事項**：
   - 錯誤記錄 → 直接寫入 `docs/error_experiences.md`
   - Traffic 記錄 → 直接更新 `revamp/outputs/execution-log--*.md`
   - SEO 修復 → 直接執行 `npm run seo:fix`
   - Git 操作 → 直接 commit + push
...
**教訓**：「執行完整流程」= 完成所有步驟 + 所有收尾工作，不需要中途確認，不需要使用者提醒。
